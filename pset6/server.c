...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
	    ...
	    memset(absolutePath,'\0',80);	
	    i = 0, j = 0;
	    while(line[i]!='/')
		i++;
	    while(line[i]!=' ' && i<strlen(line) && line[i]!='?')
		{
		absolutePath[j++]=line[i++];			
		}
	    ...
	    int k = 0;
	    char extension[40];
	    while(absolutePath[k]!='.')
		k++;
	    //printf("abs length : %d %d\n",strlen(absolutePath), k+1);
	    memset(extension,'\0',40);
	    for(i=k+1;i<strlen(absolutePath);i++)
		{
		extension[j++]=absolutePath[i];
		}
            printf("Extension - %s\n",extension);
...
bool error(unsigned short code)
{
    ...
    printf("\033[31m");
    printf("HTTP/1.2 %i %s", code, phrase);
    printf("\033[39m\n");
